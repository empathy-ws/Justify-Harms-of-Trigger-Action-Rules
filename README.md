...
python crawler.py
```
Sentence embeddings of descriptions are calculated by SentenceBERT, whereas we use the cosine similarity to compare the embeddings. The size of  embeddings in the small version of GPT-2 is 768, so we build 768-dimensional representations. Skip-Gram representations are calculated by running the following command
```

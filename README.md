...
python crawler.py
```
Sentence embeddings of descriptions are calculated by SentenceBERT, whereas we use the cosine similarity to compare the embeddings. The labeled pairs (target service, context service) are calculated by setting a threshold $\phi$ and running the following command
```
python create_dataset.py
```
Finally, Skip-Gram representations are calculated by running the following command
```

...
import pandas as pd
import ftfy
from module import PythiaPromptLearning
from transformers import AutoTokenizer, AdamW
from utils import Batchify, now_time
...
parser.add_argument('--outf', type=str, default='generated_prompt/generated_pythia_span_' + threshold + '_' + model_size + '.txt',
...
model_path = os.path.join(args.checkpoint, 'pythia_span_' + threshold + '_' + model_size + '.pt')
...
eos = '<eos>'
pad = '<pad>'
tokenizer = AutoTokenizer.from_pretrained("EleutherAI/pythia-" + model_size + "-deduped", bos_token=bos, eos_token=eos, pad_token=pad)
...
train_data = Batchify(train_df, tokenizer, bos, eos, args.words, word2id, args.batch_size, shuffle=True)
val_data = Batchify(val_df, tokenizer, bos, eos, args.words, word2id, args.batch_size)
...
model = PythiaPromptLearning.from_pretrained("EleutherAI/pythia-" + model_size + "-deduped", weights)

...
parser.add_argument('--outf', type=str, default='generated_prompt/generated_pythia_qa_' + threshold + '_' + model_size + '.txt',
...
model_path = os.path.join(args.checkpoint, 'pythia_qa_' + threshold + '_' + model_size + '.pt')
...
def generate_qa_model(rule):
    ...
    with torch.no_grad():
        ...
        features = 'Why might rule ' + 'input=[' + 'triggerTitle=' + ftfy.fix_text(
            rule[2]) + '; actionTitle=' + ftfy.fix_text(
            rule[3]) + '; title=' + ftfy.fix_text(
            rule[4]) + '; desc=' + ftfy.fix_text(
            rule[5]) + ']' + ' cause a ' + convert_class(
            rule[6]) + ' harm? '
...
for i in tqdm(range(0, len(test_df))):
    ...
    print(rule)
    idss_predicted = generate_qa_model(rule)
